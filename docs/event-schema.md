...
**새 탭 둘 — 대시보드·보고서 (결정 81·92, M6) — 계약 초안.** 도면 1판(결정 100, 09-14 08:41 PASS)과 하영 낱말 확정본(opsroom-words.md 7절) 뒤에 쓴다.
**화면 모양은 헨리 피그마 시안 뒤**(결정 93) — 여기는 두 탭이 **무엇을 읽나**만. 탭 이름 자체는 대표가 0-1 을 고를 때까지 임시(하영 사전).
두 탭은 대표가 고른 질문 둘을 나눠 갖는다 — **대시보드 = "뭐가 막혔나"(지금)**, **보고서 = "누가 뭘 했나"(지난 시간)**. 둘 다 위의 목록 둘을 자른다 — 세는 코드는 `blockedOf`·`doneOf` 하나씩, 화면은 자르기만.

| 탭 | 읽는 것 | 어떻게 자르나 | 없을 때 |
| --- | --- | --- | --- |
| 대시보드 | `blockedOf(...)` 전부 + `summaries` 의 팀 다섯 줄 | 위 = `waitOn:'boss'` 만(정하실 것 — 종·"내가 할 일"·타일이 **같은 목록**을 본다) · 가운데 = 나머지 `waitOn`(톰·팀장·운영 — 누가 움직여야 하나) · 아래 = 팀 다섯 줄(알약 다섯 글자) · `infra` 는 `state!=='ok'` 일 때만 맨 위 띠 | "지금 정하실 것 없어요. 팀이 일하는 중이에요." |
| 보고서 | `doneOf` 를 창으로(오늘 0시~ · 어젯밤 = 어제 18시~오늘 9시 · 날짜 고름) + 팀마다 `progress.next[]` + `out/**` 그림(`listOut`, 창 안 mtime) + 대리 결정(`note meta.proxy`) + 톰 글(`teams/hq/out/reports/<날짜>.md`, 있으면) | 결정 80 의 다섯 절 순서 — ① 사이 있었던 일(팀마다 `doneOf` 를 팀별로 접어 한 줄 + 펼치면 목록) ② 대표님이 보실 것(= 대시보드 위 칸과 같은 목록) ③ 오늘 각 방이 할 일(`progress.next`) ④ 톰의 한마디(파일 있는 날만) ⑤ 사이 나온 그림(`out/` png 썸네일) + 맨 위 "대표님 대신 정한 것"(대리 결정, 결정 85 ③) | "이 사이엔 한 일이 없어요." |

서버 문: `GET /api/dashboard` → `{ blocked: blockedOf 결과, teams: summaries 요약 줄, infra }` · `GET /api/report?since&until` → `{ done: [팀별], next: {팀: []}, images: [], proxy: [], chief: md|null }`.
둘 다 **파일을 새로 만들지 않는다** — 있는 것을 읽어 조립할 뿐. 톰 글(④)이 없어도 탭은 뜬다(아래층이 정본, 톰 글은 그 위 한 문단 — `out/m6-screen-inventory.md` 3절).
관제탑 "전체" 의 타일·"내가 할 일"·팀 줄은 대시보드가 생기면 그쪽으로 옮기고 "전체" 는 뺀다 — 같은 것을 두 군데 두지 않는다(findings #5·#7). 순서·자리는 시안에서.

**대시보드 1판 — 예정 작업 표 (결정 128, 도면·시안 전에 먼저)**: 대표가 "예정 작업을 표로 보고 싶다" 고 직접 말해 화면 모양을 안 기다리고 먼저 뜬다.
`GET /api/dashboard` 가 지금은 `{ text, at }` 만 준다 — `text` 는 `teams/hq/out/plan-table.md` 원문(톰이 파일로 관리, 읽기만), `at` 은 그 파일의 mtime. 화면은 그 마크다운을 그대로 표로 그려 대시보드 탭 맨 위에 놓는다(`server/public/app.js mdToDom`).
위 표의 `blocked`·`teams`·`infra` 는 M6 화면이 붙을 때 **같은 응답에 필드로 더한다** — `text` 를 빼거나 경로를 바꾸지 않는다. 톰 글(plan-table.md)이 늘 위, 집계(blockedOf 등)는 그 아래.
...
않는다. 그 방이 닫혀 있으면 총괄실에 `note` 로 남기고 옮기지 않는다. 대표 발언은 옮기지 않는다 — 지시는 배달(4절)로.
