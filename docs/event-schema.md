...
| `bossCall` | `{ id, ts, text }` 또는 null | 이 라운드에서 대표에게 **결정을 청했는데**(`asksBoss` — 대표를 불렀고(`callsBoss`) 물음표·"정해 주세요·골라·답해" 가 있다, 결정 52) 그 뒤 대표가 말하지 않았다. 부르기만 한 보고("대표님, 정리했습니다.")는 여기 안 실리고 `bossNotes` 에만. `text` 는 그 발언 첫 80자 — 카드의 `"손 하나 빌려도 될까…"`. 대표가 답하면 null. 팀 요약의 `bossCall` 과 같은 판별에 `text` 만 더한 것 |
...
오늘 대표를 부른(`callsBoss`) 발언을 최근 것부터 30건까지 `{ id, ts, by, text(160자), ask }`. `ask` 는 결정이 필요한 말인가(`asksBoss` —
물음표 · "정해 주세요·골라·답해", 결정 52) — 화면은 `ask` 가 아닌 것을 보고로 그린다. **종 배지(`bossCall`)도 같은 판별이다**(M4) —
...
**보고와 결정 요청은 다르다** (결정 52) — `asksBoss(text, cast)` = 대표를 부른 **문단** 안에 물음표·"정해 주세요·골라·답해" 가 있다.
