...
**사건 → 연출** (W1 범위. 회의·도장·게시판·램프는 W3)

| 사건 | 연출 |
| --- | --- |
| `message` (자리) | 화자 위 말풍선. 80자까지, 누르면 전문. 한 사람에 최대 셋 |
| `message` (boss) | 댄이 그 방 문(총괄실은 대표실 문)에 나타나 말한다. 댄이 직접 움직이는 중이면 그 자리에서 |
| `message` (system) · `note` · `round_start` · `round_end` | 방 위에 띠 |
| `verdict` | 판정자 말풍선에 PASS/REVISE/FAIL 색 테두리 |
| `tool` | 화자 옆에 "⌨ 작업 중" 1.5초 |
...
W2(시계·루틴: 근무·점심·퇴근·밤, 사회자의 침묵 차례 스위치)와 W3(회의·도장·게시판·대표 등장·램프·자동 카메라)은 계획 파일의 C-4 를 따른다.
