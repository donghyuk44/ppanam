...
**`system` 자리의 두 사람(결정 208, 대표 09-18 10:58 "나래와 나리는 분리된거야. 두명의 각자 다른 사람이야")** — 서버 자리는 나리(`name`, 그대로), 관리 창은 나래(`cliName`, 열쇠 하나 더).
자리 열쇠(`system`)와 대화록·훅·권한은 안 갈랐다 — 화면이 그 말의 `meta.hand`(3절)로 가른다: `cli`(관리 창 손)면 `cliName`, 없거나 `server` 면 `name`. 얼굴·색·직책·사람 카드를 따로 두는 건 아직 없다(다음 손).
...
- `meta.replyTo: { event: '<evt id>', actor: '<자리>', text: '<원문 첫 줄, 60자 안>' }` — **답장**(결정 245, 대표 09-18 13:52 "어떤 누군가의 말에 특정한 답변을 하고 싶은데" — 카카오톡 답장 꼴, 보드 J33). 같은 방의 말풍선 하나를 골라 그 말에 답한 것. 대표가 화면에서 고르면 `POST /api/say` 가 `replyTo`(같은 꼴)를 받아 대표 말풍선 `meta` 에 그대로 찍고, 세션에 넣는 글 첫 줄에 `(○○의 말 "원문 첫 줄" 에 답장)` 을 붙여 자리가 어느 말에 대한 답인지 안다 — 찍는 건 서버. 자리가 답장할 땐 `say.mjs --reply <evt id>` 가 같은 값을 찍는다(뒤, 솔라). 화면은 말풍선 **위**에 원문 한 줄(`.bub__quote` — 이름 · 첫 줄, 누르면 그 말풍선으로 건너뛰어 잠깐 빛남), 접힌 채 한 줄뿐이라 펼칠 것은 없다. `event` 가 화면에 없으면(오래된 말) 줄만 서고 건너뛰기는 이전 대화를 더 읽어 찾는다(`jumpTo`).
...
`{ silent: true }` 를 같이 보내면 파일만 저장하고 `message` 이벤트도 세션 알림도 안 남긴다 —
`{ ok, name, silent: true }` 만 돌아온다(C17, 테라 요청 0918). 입력창이 여러 첨부를 들고
있다가 "보내기"를 누르면 그때 글과 합쳐 **한 말풍선**으로 나간다 — 첨부마다 따로 말풍선이
서던 것을 막는다(대표 09-18 11:23 지적).
...
닫힌 방엔 차례가 안 와서 실무가 `--next` 를 걸 수 없는 닫힌 고리 때문이다. 문(결정 자리, hq 카드면 제리도)은 그대로.
통과하면 `notifier.applyAction` 이 `now` 로 옮기고 **라운드까지 연다** — 서버가 올렸든(`autoOpen`) `bus/approve.mjs --request B --next`
로 손으로 올렸든 똑같다(대표 09-16 19:0x "그 병이 안생기게 해야겠지" — 손으로 올린 요청은 통과해도 사람이 열 때까지 멈춰 있었다). 그 방이 이미
열려 있으면 열지 않고 마일스톤 번호만 맞춘다(`action.autoOpen` 필드는 남아 있지만 이제 읽지 않는다).
