...
| 분석 | 왜 자꾸 이렇게 되나 · **뒤** | 라운드 기록(`rounds.jsonl`)·판정·승인·막힘의 되풀이 | 숫자 하나는 안 온다 — **다른 숫자와 견줄 때만**(같은 자리에서 몇 번째 막힘, 라운드 길이의 흐름). **첫 층은 그림** — 다섯 팀 같은 자(헨리 numbers.md ⑤ 작은 그림 다섯: 팀마다 진행 막대 + 오늘 시간 띠, 막힌 팀만 빨간 띠 — 나리 정본 "글자만인 첫 층은 통과가 아니다", 9단계 ③). 칸 셋 · 문장 틀 셋(하영 화면 글 틀 3절 — 어디서 자꾸 막히나 · 무엇이 느려졌나 · 같은 일이 몇 번째인가)은 헨리 분석 시안(H4) 뒤 — 그때 옛 타일·표가 빠진다. 항목은 대표가 `out/analysis-inventory.md` 에서 고른 것만(로드맵 M6) | "견줄 만큼 쌓이지 않았어요." |
