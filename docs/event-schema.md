...
- **호명 칩**(C13, 대표 09-16 11:0x "박스 안에 이름 … 멘션할 때 잘 안 보이더라"): 첫머리 `@이름`·`이름,`(mention.js)은 그 사람 색 박스(`.mention.mchip` — `.chip` 은 얼굴 칩이라 다른 이름)로, 옆에 받았나 작은 글자 — 칩 뒤에 그 사람 말풍선이 있으면 "답함", 일하는 중이면 "읽음, 답변 중", 아니면 "대기"(`app.js markMentions`, 화면에 있는 것만 본다). 입력창에서 `@` 를 치면 그 방 사람 목록(`.mpop`, 위아래·Enter·클릭)이 뜨고 고르면 `@이름, ` 이 들어간다 — 서버 판별은 그대로(글자만 넣는다).
...
서버 문: `GET /api/dashboard` → `{ text, at, now, teams: [{ id, name, room, color, hasRoadmap, owners: [{ actor, name, initial, color }], stages: [{ n, title, status, plannedFrom, plannedTo, late, blockedWhy, gate, gateWhat, gateBoss, rounds }], roundMs }], bossGates: [{ kind, team, what, id }] }`
— `hasRoadmap` 은 `teams/<팀>/roadmap.json` 이 있나(빈 줄 말 둘을 가른다). `owners` 는 그 팀 cast 에서 다른 회사(외부 감사)·대표·안내를 뺀 둘, 팀장(`roomRules.owner`) 먼저(`bus.ownersOf`, 순수 — 계획표 `owner` 칸은 마케팅만 있어 cast 가 정본, 하영 1-3). `gate` 는 timebox 원문(펼친 줄), `gateWhat` 은 칸 글자 — "대표" 가 들면 **대표님이 정한 뒤**, 아니면 조건의 **{무엇} 뒤**("1 라운드 — 테라 붙인 뒤" → "테라 붙인 뒤", `bus.gateWhatOf`), `gateBoss` 는 대표 문인가(카드 "대표님이 여실 단계" 는 이것만). `rounds` 는 timebox 회차 수(0.5 = 반).
...
`roundMs` = 그 팀 닫힌 회차 최근 여덟의 평균 길이(멈춤 뺀 것, 없으면 기본 90분). **1분 미만·일주일 넘는 회차는 시험·시드라 뺀다**(`ROUND_MIN_MS`·`ROUND_MAX_MS` — 디자인 09-12 0.2초 셋, 경영 09-01 11일짜리가 평균을 6일로 만들었다, 하영 T3). 계산은 순수 함수 `bus.plansOf({ roadmap, state, rounds, progress, now, pauses })` — `round.mjs check` 가 돌린다. 지금 단계의 `plannedFrom` 은 지금 회차 `startedAt`, `plannedTo` 는 거기서 timebox 회차 수 × `roundMs` 만큼 **일한** 시각 — 멈춘 구간은 건너뛴다(`bus.endAfterWork`: 옮긴 자리가 또 멈춤 안이면 또 민다 — 한 번만 더하면 디자인 4단계가 거짓 "1시간 22분 늦음" 이 됐다, T2); 다음 단계들은 앞 단계 `plannedTo` 뒤로 잇는다. timebox 에 조건("대표" · "… 뒤")이 있으면 `gated` — **단, 지금 단계(`status: 'now'`, 착수 카드가 통과된 것)는 문이 아니다**(문은 열렸다 — 나리 실측 09-15: 경영 2단계가 착수됐는데 "대표가 정한 뒤" 로 섰다): 글자 속 회차 수, 없으면 1 라운드. timebox 가 아예 없는 단계는 문이 아니라 **잡히지 않은 것** — `planned` 에 날짜 없음(지어내지 않는다). `blockedWhy` 가 비면 **검토에서 멈춤 — 대표님 판단 기다림**(T8).
...
나리는 서버가 띄운 세션이 아니라 환경(`PPANAM_TEAM`)이 없다 — `approve.mjs --decide <id> --as system` 은 **환경 없는 셸에서만** 받아 총괄실 사람(`system`, team hq)으로 적는다.
...
(`endRound({ next })`) 이 자동 잇기는 물러난다 — 안 그러면 그 자리의 `startRound` 가 "이미 열려 있다" 로 던진다.
