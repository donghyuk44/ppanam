...
- **호명 칩**(C13, 대표 09-16 11:0x "박스 안에 이름 … 멘션할 때 잘 안 보이더라"): 첫머리 `@이름`·`이름,`(mention.js)은 그 사람 색 박스(`.mention.chip`)로, 옆에 받았나 작은 글자 — 칩 뒤에 그 사람 말풍선이 있으면 "답함", 일하는 중이면 "읽음, 답변 중", 아니면 "대기"(`app.js markMentions`, 화면에 있는 것만 본다). 입력창에서 `@` 를 치면 그 방 사람 목록(`.mpop`, 위아래·Enter·클릭)이 뜨고 고르면 `@이름, ` 이 들어간다 — 서버 판별은 그대로(글자만 넣는다).
