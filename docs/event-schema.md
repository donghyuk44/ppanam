...
"자는 중" 이다 — 쉬는 codex 를 "듣는 중" 으로 그리지 않는다 (레오 R15 감사). `/api/team` 의 `summary` 도 부팅·방송과 같은 모양이다(세션·차례 포함) — 얇은 것을 주면 방에 들어간 직후 일하는
...
**대표를 부른 말** — `callsBoss()`: 대표 이름 외에 "대표님·대표·댄" 도 호명이다. 요약(`teamSummary.bossCall`)에 "이
라운드에서 대표를 불렀는데 그 뒤 대표가 말하지 않은" 발언이 실리고, 화면은 그 말풍선에 멘션 표시를, 탭 줄에 "대표 차례"
배지를 띄운다 (결정 19-2).
...
**관람 (W3).** 캐릭터를 누르면 카드 — 지금(자리·상태), 누구(인격 파일의 첫 줄과 "## 너라는 사람" 절), 일지 맨 위 문단, 최근 발언 여덟(누르면 작전실의 그 발언으로), 말 걸기. `GET /api/actor?team&actor`.
