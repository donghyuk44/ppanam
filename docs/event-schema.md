...
- **산출물** `artifacts` — 그 요청이 가리키는 `out/` 파일들 (대표 결정 36 — "디자인 그림이 없는데 내가 어떻게 승인해").
  ...
  모르면서 승인하게 두지 않는다.
- **대표 단추** 확인 · 돌려보냄은 C 카드에만. **위임 중**(`boot.delegation`, `notify.delegated` — 결정 136)이고 카드가 대리 가능
  (`proxyable`, 돈·바깥 아님)이면 단추 자리에 **지금은 톰·제리가 정해요** 한 줄(`.apr__proxy`) + 안내 **되돌리시려면 방에 한마디.** —
  위임 중엔 10분 규칙이 아니라 바로 대리되므로 "10분 안 누르시면" 안내는 안 보인다. 돈·바깥 C 는 위임 중에도 단추 그대로. R31 ②, 나리.
