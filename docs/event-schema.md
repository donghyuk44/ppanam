...
| **B 작은** (`--small`, 레코드 `small: true`) | 재시작 · 문구 한 줄 · 임시 파일 태그 — **실행 대상 없는 B 만**(`--push`·`--next`·`--roadmap`·`--to` 와 못 씀, C 는 안 됨) | **톰 혼자**(`bus.needsOf` — 제리 대조 생략, 점검-0916 3-9 "감사 무게 나누기": 밤 99건 중 재시작 15장에도 둘 다 대조). 작은 게 아니면 톰이 REVISE 로 돌려보내 큰 카드로 | 톰이 답할 때까지 |
