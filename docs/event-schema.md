...
| 5 (브레이크 없음) | 두 사람 왕복 3회 제한은 **결정 120 으로 철회**(대표 09-14 "3번 넘으면 대화 못하게 하는 거 철회해" · 09-02 "대화로 풀어가라"). 호명은 몇 번을 오가도 그대로 준다. 남은 상한은 4 의 침묵 차례 시간당 30 뿐 — **호명 왕복은 그 수에 안 든다**(`underCap` 은 `lulls` 만 센다). 한 자리에 한 번에 한 턴 — 일하는 중이면 쌓이고, 쌓인 차례는 합쳐서 한 턴 |
