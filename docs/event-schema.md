...
`blocked` = `bus.blockedSpansOf(log, cast, { team, since, until, now })`(순수, check) — `[{ team, teamName, kind: 'fail'|'ask', from, to|null, text, by, name, ref }]`: FAIL 구간(note `meta.blocked` → note `meta.resumed`) · 답 없는 물음(`asksBoss` 말 → 그 뒤 첫 대표 말(via 제외)/대리 답, 라운드가 바뀌면 열린 채 닫힘) · 창과 겹치는 것만 · 총괄실·비서실은 안 센다(대표와 1:1). `teams` = 방마다 `{ id, name, room, color, office, milestone, milestoneTitle, round, done, total, phase, guide }`(비서실 뺌). 시각 글자는 `whenKo` — "아침 8:41 · 낮 12:13 · 저녁 6시 · 밤 10:50"(3-5-1, 우리 시각).
