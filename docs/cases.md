...
| 34 | 우리 09-12 | 외부감사가 두 번째 턴부터 매번 "외부 모델을 부르지 못했습니다" — 첫 턴만 되고 기억이 리셋 | `codex exec resume` 이 `--sandbox`·`-m`·`-o` 를 안 받는데 새 세션과 같은 플래그를 줬다. 실패 처리가 세션을 지우니 다음 턴은 새 세션으로 "성공"해 고장이 안 보였다 | resume 은 `-c model=…`·`-c sandbox_mode=read-only` 로, 답은 stdout 으로. 두 경로를 다 실측하고 커밋 메시지에 적는다 |
