...
- `teams/design/out/screens/ui/png/<화면>.png` — 시안 그림. 지금 고치는 화면 한 장만 연다(tower·dashboard 4번, analysis 3번, report 2번 읽힘).
- `teams/design/out/kit/characters.json` (185줄 · 11KB) — 얼굴·인형 색표. 자리 하나 볼 땐 `grep -n '"<자리>"'`.
- `server/public/style.css` (1117줄 · 73KB) — 실물 색·간격. **전문 금지** — 화면 칸 이름(`.report`·`.tower`)으로 `grep -n` 한 뒤 그 블록만. `card.css`(117줄)는 통째로 돼도 된다.
...
- `teams/hq/out/카드-체계-0916.md`(65줄) 는 짧다 — 이건 통째 가능.
