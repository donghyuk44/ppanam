...
- `teams/marketing/out/opsroom-words.md` (607줄 · 103KB · 절 12) — 낱말 사전 정본. **하루에 12번 읽힌 파일** —
  전문이 아니라 `grep -n '^## '` 로 절을 찾고 그 절만(`Read offset/limit`). 낱말 하나 찾을 땐 `grep -n '<낱말>'`.
...
- `teams/dev/out/shots/*.png` — 화면 사진은 나리가 판독해 카드에 붙인다. 30초 시험 때 자기 화면 하나만.
