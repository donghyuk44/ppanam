...
import { notificationsOf, blockedOf, pausedMs, delegated, deciders, URGENT } from '/notify.js';
...
// 설정 › 알림에서 끈 방(mutedTeams, 이 브라우저만)의 항목은 종 목록·배지에서 뺀다 — 셈은 notify.js 와 같은 식(unread = 안 읽은 대표 몫, urgent = 그중 결재·답변·차단).
const notifications = () => {
  const r = notificationsOf({ teams, summaries, approvals }, { read: readIds(), delegation });
  const mute = mutedTeams(); if (!mute.size) return r;
  const items = r.items.filter((it) => !mute.has(it.team));
  return { items, unread: items.filter((it) => it.unread && it.mine).length, urgent: items.some((it) => it.unread && it.mine && URGENT.has(it.kind)) };
};
...
matchMedia('(min-width: 820px)').addEventListener('change', () => { placeSetNav(); if (view === 'settings') renderSetIn(); });   // 어느 화면에서든 자리를 옮긴다 — 폰 방 목록 서랍에 설정 목록이 남지 않게
...
async function renderSetIn() {
  ...
  // 화면마다 새 상자 — 읽는 중(fetch)에 다른 줄을 누르면 늦게 온 것은 떨어진 상자에 붙어 안 보인다(code-review — 멤버 → 알림 사이 겹침)
  const box = el('div', 'setin__box'); inner.appendChild(box);
  if (fn) await fn(box);
}
...
// '대표님이 시킨 일' 첫 층(34회차, 하영 ahead-five.md 손 글 다섯 줄)의 loadAheadFive 는 뺐다 — 타임라인 3판 시안에 없고 손 글 칸이라(계획 6절 컷, 나리 대리 결정 · 헨리 09-18). 그 자리는 목표 한 장(goalCards).
