    ...
    try {
      ...
      // out/ 그림이 있으면 썸네일, 읽음 집합에 있으면 unread:false. 숫자·빨강은 대표 손이 필요한 것(mine)만 — 보고·B 는 패널에만(나리 결정 ② 09-15).
      {
        ...
        const nWant = order === 'boss:e1,approval:theirs,approval:apr_1,blocked:design,report:e5,report:e0' && n1.unread === 3 && n1.urgent
          ...
          && n1.items[1].text === '톰·제리가 보는 중 1건' && n1.items[2].text === '로드맵 교체' && n1.items.map((i) => +i.mine).join('') === '101100'   // '· 10분 안' 은 결정 188 로 뺌
          && n2.unread === 0 && !n2.urgent && n2.items.filter((i) => i.unread).length === 3;
        out.push(['알림 목록(결정 68 · 나리 ② · U3·U4)', nWant ? '✓ 종류 순 6건 · B 는 "톰·제리가 보는 중 1건" 한 줄 · C 는 무엇(10분 안 은 188 로 뺌) · ask 제외 · 썸네일 · 숫자는 mine 셋 · 읽음 뒤 0·urgent 꺼짐(안 읽은 보고·접힌 줄 셋은 숫자 밖)' : '✗ ' + JSON.stringify({ order, unread: n1.unread, urgent: n1.urgent, texts: n1.items.map((i) => i.text), mine: n1.items.map((i) => i.mine), n2: [n2.unread, n2.urgent] })]);
